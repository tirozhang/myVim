...
let s:PATH_SEP	= ':'
let s:FILE_SEP	= '/'
if has("win32") || has("win64") || has("win16") || has("dos32") || has("dos16") || has("win32unix")
  let s:PATH_SEP	= ';'
  let s:FILE_SEP	= '\'
endif
  ...
  if b:context_type != s:CONTEXT_PACKAGE_DECL
    ...
    try
      ...
      try
        let filepatterns = ''
        for dirpath in s:GetSourceDirs(expand('%:p'))
          let filepatterns .= escape(dirpath, ' \') . '/*.groovy '
        endfor
        exe 'vimgrep /\s*' . s:RE_TYPE_DECL . '/jg ' . filepatterns
      finally
      endtry
...
fu! javacomplete#AddClassPath(s)
  ...
  if has("win32unix") 
    let @a = system("cygpath -w '".a:s."'")
    let dir = substitute(@a,"^\\s\\+\\|\\s\\+$","","g") 
  else
    let dir = a:s
  endif
  ...
  if !exists('s:classpath')
    let s:classpath = [dir]
  elseif index(s:classpath, dir) == -1
    call add(s:classpath, dir)
  endif
  let s:cache = {}
endfu
...
" s:GetClassPath()							{{{2
fu! s:GetClassPath()
  let path = $CLASSPATH.s:PATH_SEP.s:GetJavaCompleteClassPath() . javacomplete#GetClassPathSep()
...
fu! s:GetJavaCompleteClassPath()
  ...
  if has("win32unix") 
    let @a = system("cygpath -w '".fnamemodify(classfile, ':p:h')."'")
