...
function! s:Project(filename) " <<<
    ...
    if exists("g:proj_running")
        ...
        if strlen(a:filename) == 0
            if filereadable('.vimproject')
                let filename ='./.vimproject'      " Default project filename
            else
    ...
    if !exists('g:proj_window_increment')
        let g:proj_window_increment=200         " Project Window width increment
    endif
    ...
    if !exists("g:proj_running") || (bufwinnr(g:proj_running) == -1) " Open the Project Window
        ...
        if match(g:proj_flags, '\CF') == -1      " We're floating
            silent! wincmd H
            exec 'silent vertical resize '.g:proj_window_width
        endif
        setlocal nomodeline
    else
        silent! 99wincmd h
    ...
    let g:proj_locate_command='silent! wincmd H'
    let g:proj_resize_command= 'exec ''silent vertical resize ''. g:proj_window_width'
'
    if match(g:proj_flags, '\CF') != -1         " Set the resize commands to nothing
        let g:proj_locate_command=''
        let g:proj_resize_command=''
    endif
    ...
    function! s:DoSetup()
        setlocal foldenable foldmethod=marker foldmarker={,} commentstring=%s foldcolumn=0 nonumber noswapfile shiftwidth=2
        setlocal foldtext=ProjFoldText() nobuflisted nowrap
        setlocal winwidth=1
        setl statusline=%<%f\ %3*%m%1*%r%0*
    ...
    setlocal indentexpr=GetProjectIndent()
    setlocal indentkeys+={
    ...
    " s:FindBiggestWindow() <<<
    function! s:FindBiggestWindow(n)
        " 寻找一个面积最大的Windows
        let l:count = winnr('$')
        let l:i = 0
        let l:big = 0
    ...
    function! s:DoSetupAndSplit()
        ...
        let l:id = s:FindBiggestWindow(n)
        exec l:id."wincmd w"
        ...
        if n == winnr()
            ...
            exec g:proj_locate_command
            exec g:proj_resize_command
      ...
      " if filereadable(home . "/.proj.vim")
      "     exec "so ".home "/.proj.vim"
      " endif
    endfunction ">>>
    ...
    if !exists("g:proj_running")
        ...
        nnoremap <buffer> <silent> <C-Return> \|:call <SID>DoFoldOrOpenEntry('', 'vsp')<CR>
        ...
        nnoremap <buffer> <silent> <C-k>   \|:silent call <SID>MoveUp()<CR>
        nnoremap <buffer> <silent> <C-Down> \|:silent call <SID>MoveDown()<CR>
        nnoremap <buffer> <silent> <C-j>   \|:silent call <SID>MoveDown()<CR>
        ...
        exec 'au FileType '.bufname.' call s:Highlight()'
        " >>>
        setlocal buflisted
    ...
    function! s:Highlight()
        if match(g:proj_flags, '\Cs')!=-1 && has('syntax') && exists('g:syntax_on')
            ...
            syntax match projectFoldMarker       '{'
            syntax match projectFoldMarker       '}'

            highlight def link projectDescription  Question
            ...
            highlight def link projectFoldMarker   Keyword
        endif
    endf ">>>
    call s:Highlight()
endfunction " >>>
...
if exists(':Redraw') != 2
    command Redraw call <SID>Highlight()
endif
" Toggle Mapping
