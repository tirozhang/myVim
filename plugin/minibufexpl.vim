...
augroup MiniBufHighlight
    autocmd!
    autocmd FileType -MiniBufExplorer- call <SID>Highlight()
augroup END
...
function! <SID>FindWindow(bufName, doDebug)
  if a:doDebug
    call <SID>DEBUG('Entering FindWindow()',20)
  endif
  ...
  if l:bufNum != -1
      ...
      if a:doDebug
          call <SID>DEBUG('Found buffer ('.a:bufName.'): '.l:bufNum.'('.l:winNum.')',20)
      endif
  else
      if a:doDebug
          call <SID>DEBUG('Not found buffer ('.a:bufName.')',20)
      endif
      let l:winNum = -1
  endif
  ...
  if g:miniBufExplorerAutoUpdate == 1
    ...
    if ((g:miniBufExplorerMoreThanOne == 0) || (bufnr('%') != -1 && bufname('%') != ""))
      if <SID>HasEligibleBuffers(a:delBufNum) == 1
        ...
        let l:bufnr = <SID>FindWindow('-MiniBufExplorer-', 1)
        if (l:bufnr == -1)
