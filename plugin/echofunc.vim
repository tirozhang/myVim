...
function! EchoFunc()
    let rets = '('
    let cols = 2
    if getline('.')[(col('.')-2):(col('.')-2)] == '('
        let rets = ''
        let cols = 3
        echo getline('.')[:(col('.')-cols)]
    endif

    let name=s:GetFuncName(getline('.')[:(col('.')-cols)])
    if name==''
        return rets
    endif
...
function! s:EchoFuncInitialize()
    augroup EchoFunc
        ...
        autocmd BufRead,BufNewFile *.cpp call CheckedEchoFuncStart()
        if has('gui_running')
